...
export function enLettres(value: number): string {
  ...
  // sf (scaleFollows) = le groupe est suivi de « mille » : « cent » et « vingt »
  // restent alors invariables (six cent mille, quatre-vingt mille). Devant
  // « million(s) » (nom), l'accord est conservé : sf = false.
  const below100 = (x: number, sf: boolean): string => {
    ...
    let w = tens[t];
    if (u === 0) return (t === 8 && !sf) ? w + 's' : w;
    if (u === 1 && t !== 8) return w + ' et un';
    return w + '-' + units[u];
  };

  const below1000 = (x: number, sf: boolean): string => {
    if (x < 100) return below100(x, sf);
    const c = Math.floor(x / 100), r = x % 100;
    const w = c === 1 ? 'cent' : units[c] + ' cent';
    if (r === 0) return (c > 1 && !sf) ? w + 's' : w;
    return w + ' ' + below100(r, sf);
  };
  ...
  const rest = m;
  if (millions) parts.push(millions === 1 ? 'un million' : below1000(millions, false) + ' millions');
